	...
	public void run() {
		...
		for(String blkId : finalList){
			...
			for(String dataNode : dataNodes){
				try{
					String newIpAddress = NameNode.getNodeList(3).get(1);
					sendReply(dataNode,Constants.DATANODE_PORT,blkId,newIpAddress);
					NameSpaceTree.update(blkId,report.getIpAddress(),newIpAddress);
					break;
